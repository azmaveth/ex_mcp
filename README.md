...
> **Body parsing:** a Phoenix endpoint runs `Plug.Parsers` before the router, so a plug mounted in the router only ever sees an already-drained body. `ExMCP.HttpPlug` handles that by falling back to the payload Phoenix already parsed (`conn.body_params`), so the mount above works as-is. A non-JSON request, a JSON-RPC batch (a top-level array, which Phoenix wraps as `%{"_json" => [...]}`), or strict enforcement of `HttpPlug`'s `:body_limit` still needs raw bytes stashed in `conn.assigns[:raw_body]` through a `Plug.Parsers` `:body_reader`; alternatively, set the parser's `:length` to the same limit. Mounting the plug in the endpoint ahead of `Plug.Parsers` works too, but then it has to match its own path and `halt/1` the conn itself, since `ExMCP.HttpPlug` sends its response without halting.
