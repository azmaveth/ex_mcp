...
> **Body parsing:** a Phoenix endpoint runs `Plug.Parsers` before the router, so a plug mounted in the router only ever sees an already-drained body. `ExMCP.HttpPlug` handles that by falling back to the payload Phoenix already parsed (`conn.body_params`), so the mount above works as-is. Two cases still need the raw bytes to reach the plug, and both are served by stashing them in `conn.assigns[:raw_body]` through a `Plug.Parsers` `:body_reader`: a non-JSON request content type, and a JSON-RPC batch (a top-level array, which Phoenix wraps as `%{"_json" => [...]}`). Mounting the plug in the endpoint ahead of `Plug.Parsers` works too, but then it has to match its own path and `halt/1` the conn itself, since `ExMCP.HttpPlug` sends its response without halting.
