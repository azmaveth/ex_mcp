  ...
  for adapter <- ExMCP.Test.HTTPAdapter.adapters() do
    describe "with #{adapter}" do
      setup do
        ...
        {:ok, port: port, adapter: unquote(adapter)}
      end
      ...
      test "rate limiting and throttling work correctly", %{adapter: adapter} do
        port = ExMCP.Test.HTTPAdapter.free_port()
      ...
      test "server recovers from handler errors", %{adapter: adapter} do
        error_port = ExMCP.Test.HTTPAdapter.free_port()
