defmodule ExMCP.VersionRegistryTest do
  ...
  alias ExMCP.{Protocol, VersionRegistry}
  ...
  describe "version registry" do
    ...
    test "identifies latest stable version" do
      assert VersionRegistry.latest_version() == "2025-03-26"
    end
    ...
    test "returns version-specific capabilities" do
      ...
      # Draft capabilities
      caps_draft = VersionRegistry.capabilities_for_version("draft")
      assert caps_draft.tools.outputSchema
      assert caps_draft.experimental.elicitation
      assert caps_draft.experimental.structuredContent
      assert caps_draft.experimental.toolOutputSchema
    end
    ...
    test "checks feature availability by version" do
      ...
      assert VersionRegistry.feature_available?("draft", :structured_content)
    ...
    test "negotiates protocol versions" do
      ...
      assert {:error, :version_mismatch} =
               VersionRegistry.negotiate_version("draft", ["1.0.0"])
    end
  ...
  describe "protocol version-specific methods" do
    ...
    test "validates message version compatibility" do
      ...
      assert :ok = Protocol.validate_message_version(%{"method" => "elicitation/create"}, "draft")
