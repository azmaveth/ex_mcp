defmodule ExMCP.OAuthProtectedResourceMetadataTest do
  ...
  describe "Protected Resource Metadata - WWW-Authenticate Header" do
    test "extracts metadata URL from WWW-Authenticate header" do
      ...
      www_authenticate = ~s(Bearer realm="https://auth.example.com", as_uri="https://auth.example.com/.well-known/oauth-authorization-server")
    ...
    test "handles WWW-Authenticate with error information" do
      www_authenticate = ~s(Bearer realm="https://auth.example.com", error="invalid_token", error_description="The access token expired")
    ...
    test "extracts resource metadata endpoint from header" do
      www_authenticate = ~s(Bearer realm="https://auth.example.com", resource_uri="https://api.example.com/.well-known/oauth-protected-resource")
      
      result = ProtectedResourceMetadata.parse_www_authenticate(www_authenticate)
      
      assert {:ok, auth_info} = result
      assert auth_info.resource_uri == "https://api.example.com/.well-known/oauth-protected-resource"
    end
    ...
    test "handles malformed WWW-Authenticate header" do
      malformed_headers = [
        "",
        "NotBearer",
        "Bearer",  # Missing parameters
        ~s(Bearer realm),  # Missing value
        ~s(Bearer realm="unclosed),  # Unclosed quote
      ]
    ...
    test "handles 401 Unauthorized with WWW-Authenticate" do
      ...
      case result do
        ...
        {:ok, metadata} ->
          ...
          assert is_list(metadata.authorization_servers)
        other -> flunk("Expected proper 401 handling: #{inspect(other)}")
      end
    end
  ...
  describe "Protected Resource Metadata - Edge Cases" do
    ...
    test "handles concurrent discovery requests" do
      ...
      unique_results = Enum.uniq_by(results, fn result ->
        case result do
          {:ok, _} -> :success
          {:error, type} -> type
        end
