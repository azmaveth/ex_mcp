defmodule ExMCP.OAuthDynamicClientRegistrationTest do
  ...
  describe "Dynamic Client Registration - Field Validation" do
    test "validates required fields are present" do
      ...
      assert {:error, {:missing_required_fields, fields}} = ClientRegistration.register_client(request1)
      ...
      assert {:error, {:missing_required_fields, fields}} = ClientRegistration.register_client(request2)
      ...
      assert {:error, {:missing_required_fields, fields}} = ClientRegistration.register_client(request3)
    ...
    test "validates redirect URIs are HTTPS" do
      ...
      assert {:error, {:invalid_redirect_uris, invalid_uris}} = ClientRegistration.register_client(request)
  ...
  describe "Dynamic Client Registration - Response Handling" do
    test "parses successful registration response" do
      ...
      case result do
        {:ok, client_info} ->
          ...
          assert is_integer(client_info.client_secret_expires_at) or is_nil(client_info.client_secret_expires_at)
          assert is_binary(client_info.registration_access_token) or is_nil(client_info.registration_access_token)
          assert is_binary(client_info.registration_client_uri) or is_nil(client_info.registration_client_uri)
    ...
    test "handles registration errors from server" do
      ...
      case result do
        ...
        other -> flunk("Unexpected error format: #{inspect(other)}")
      end
    end
    ...
    test "requires client_id in response" do
      ...
      case result do
        {:error, {:request_failed, _}} -> assert true
        {:error, %KeyError{}} -> assert true  # Missing client_id
  ...
  describe "Dynamic Client Registration - Client Information Management" do
    test "retrieves client information with registration access token" do
      ...
      result = ClientRegistration.get_client_information(registration_client_uri, registration_access_token)
    ...
    test "updates client information with registration access token" do
      ...
      result = ClientRegistration.update_client_information(registration_client_uri, registration_access_token, updates)
    ...
    test "requires HTTPS for client management URIs" do
      ...
      assert {:error, :https_required} = ClientRegistration.get_client_information(http_uri, "token")
      
      # update_client_information with HTTP
      assert {:error, :https_required} = ClientRegistration.update_client_information(http_uri, "token", %{})
    end
  ...
  describe "Dynamic Client Registration - Security Requirements" do
    test "validates registration endpoint is secure" do
      ...
      assert match?({:error, {:request_failed, _}}, result)  # Network error, not security error
  ...
  describe "Dynamic Client Registration - Edge Cases" do
    ...
    test "handles Unicode in various fields" do
      request = %{
        registration_endpoint: "https://auth.example.com/register",
        client_name: "クライアント名前",  # Japanese
        redirect_uris: ["https://localhost:8080/callback"],
        client_uri: "https://例え.テスト/",  # IDN domain
