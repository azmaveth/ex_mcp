defmodule ExMCP.OAuthHTTPTransportIntegrationTest do
  ...
  describe "OAuth with SSE Transport" do
    ...
    test "automatically refreshes token on SSE reconnect", %{oauth_config: config, mock_token: token} do
      # Start token manager with short-lived token
      short_token = Map.put(token, "expires_in", 5)
      
      {:ok, token_manager} = TokenManager.start_link(
        auth_config: config,
  ...
  describe "OAuth with HTTP Transport" do
    test "includes Bearer token in HTTP request headers", %{mock_token: token} do
      ...
      {:ok, transport} = ExMCP.Transport.HTTP.connect(
  ...
  describe "OAuth with WebSocket Transport" do
    test "includes Bearer token in WebSocket connection headers", %{mock_token: token} do
      ...
      # Connect with auth
      result = ExMCP.Transport.WebSocket.connect(
        url: transport_config.url,
        headers: transport_config.headers
      )
    ...
    test "handles WebSocket reconnection with refreshed token", %{oauth_config: config, mock_token: token} do
  ...
  describe "OAuth Integration with MCP Client" do
    test "MCP client uses OAuth token across transport types", %{oauth_config: config, mock_token: token} do
    ...
    test "MCP client automatically refreshes tokens during operation", %{oauth_config: config} do
      ...
      client_config = [
        ...
        auth_config: config,
        initial_token: short_token,
        refresh_window: 5  # Refresh 5 seconds before expiry
      ]
  ...
  describe "OAuth Error Recovery" do
    test "retries requests after token refresh" do
      ...
      result = case make_request_with_retry.("old-token") do
        {:error, {:http_error, 401, _}} ->
