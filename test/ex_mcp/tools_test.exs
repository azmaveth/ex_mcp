defmodule ExMCP.ToolsTest do
  ...
  setup do
    # Start server with tools handler using BEAM transport
  ...
  describe "tools functionality" do
    ...
    test "client can call calculation tool", %{client: client} do
      # Addition
      {:ok, result} = Client.call_tool(client, "calculate", %{
        "operation" => "add",
        "a" => 10,
        "b" => 5
      })
      ...
      # Multiplication
      {:ok, result2} = Client.call_tool(client, "calculate", %{
        "operation" => "multiply",
        "a" => 7,
        "b" => 8
      })
    ...
    test "client can call string manipulation tool", %{client: client} do
      {:ok, result} = Client.call_tool(client, "string_tools", %{
        "operation" => "uppercase",
        "text" => "hello world"
      })
      assert hd(result.content).text == "HELLO WORLD"
      
      {:ok, result2} = Client.call_tool(client, "string_tools", %{
        "operation" => "reverse",
        "text" => "MCP"
      })
    ...
    test "tool can return different content types", %{client: client} do
      ...
      # Image content
      {:ok, result} = Client.call_tool(client, "image_process", %{
        "operation" => "resize",
    ...
    test "tool handles optional parameters with defaults", %{client: client} do
      ...
      {:ok, result} = Client.call_tool(client, "data_fetch", %{
        "source" => "database",
      ...
      {:ok, result2} = Client.call_tool(client, "data_fetch", %{
        "source" => "api",
        "query" => "users",
        "format" => "csv"
      })
    ...
    test "tool call with progress token", %{client: client} do
      ...
      {:ok, result} = Client.call_tool(client, "async_task", %{
        "duration" => 1,
        "steps" => 2
