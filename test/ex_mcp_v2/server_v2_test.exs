defmodule ExMCP.ServerV2Test do
  ...
  describe "callback implementations" do
    ...
    test "handle_tool_call with optional arguments", %{server: server} do
      ...
      {:ok, result, _state} = TestServer.handle_tool_call("hello", %{"name" => "Bob", "style" => "formal"}, state)
    ...
    test "handle_resource_read for config", %{server: server} do
      ...
      {:ok, contents, new_state} = TestServer.handle_resource_read("config://app", "config://app", state)
    ...
    test "handle_resource_read for users", %{server: server} do
      ...
      {:ok, contents, _state} = TestServer.handle_resource_read("data://users", "data://users", state)
    ...
    test "handle_prompt_get with arguments", %{server: server} do
      ...
      {:ok, result, new_state} = TestServer.handle_prompt_get("greeting", %{"style" => "casual", "language" => "spanish"}, state)
  ...
  describe "integration with content helpers" do
    ...
    test "json content helper works" do
      ...
      {:ok, contents, _state} = TestServer.handle_resource_read("config://app", "config://app", state)
