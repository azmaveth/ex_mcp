defmodule ExMCP.Reliability.SupervisorTest do
  ...
  describe "create_reliable_client/2" do
    ...
    test "creates basic reliable client without reliability features", %{supervisor: supervisor} do
      ...
      assert {:ok, client_pid} = ReliabilitySupervisor.create_reliable_client(
        supervisor, 
    ...
    test "handles transport start failure gracefully", %{supervisor: supervisor} do
      ...
      assert {:error, _reason} = ReliabilitySupervisor.create_reliable_client(
        supervisor,
    ...
    test "generates unique client IDs for multiple clients", %{supervisor: supervisor} do
      assert {:ok, client1} = ReliabilitySupervisor.create_reliable_client(
        supervisor,
        transport: [type: :mock, server_pid: self()]
      )
      
      assert {:ok, client2} = ReliabilitySupervisor.create_reliable_client(
        supervisor,
        transport: [type: :mock, server_pid: self()]
      )
  ...
  describe "ClientWrapper integration" do
    ...
    test "wrapped client forwards MCP operations", %{supervisor: supervisor} do
      assert {:ok, wrapper_pid} = ReliabilitySupervisor.create_reliable_client(
        supervisor,
        transport: [type: :mock, server_pid: self()]
      )
  ...
  describe "ExMCP.Reliability.with_retry/2" do
    ...
    test "retries failing function" do
      ...
      result = Reliability.with_retry(fn ->
    ...
    test "respects max attempts limit" do
      ...
      result = Reliability.with_retry(fn ->
        ...
        {:error, :always_fails}
      end, max_attempts: 3, base_delay: 1)
  ...
  describe "ExMCP.Reliability.protect/2" do
    test "creates circuit breaker protected function" do
      ...
      protected_fn = Reliability.protect(fn -> {:ok, "success"} end, 
    ...
    test "circuit breaker trips on repeated failures" do
      ...
      protected_fn = Reliability.protect(fn ->
        Agent.update(agent, &(&1 + 1))
        {:error, :service_down}
      end, failure_threshold: 3, name: String.to_atom("test_protect_#{:erlang.unique_integer()}"))
      ...
      assert final_count == 3  # Only the first 3 calls that opened the circuit
    ...
    test "creates unique circuit breakers for different functions" do
      protected_fn1 = Reliability.protect(fn -> {:ok, "fn1"} end,
        name: String.to_atom("test_fn1_#{:erlang.unique_integer()}"))
      protected_fn2 = Reliability.protect(fn -> {:ok, "fn2"} end,
    ...
    test "handles function arguments correctly" do
      protected_fn = Reliability.protect(fn -> {:ok, 8} end,
    ...
    test "respects circuit breaker configuration" do
      ...
      protected_fn = Reliability.protect(fn ->
        Agent.update(agent, &(&1 + 1))
        {:error, :fail}
      end, failure_threshold: 1, name: String.to_atom("test_config_#{:erlang.unique_integer()}"))
  ...
  describe "integration scenarios" do
    ...
    test "multiple reliable clients can coexist", %{supervisor: supervisor} do
      ...
      assert {:ok, client3} = ReliabilitySupervisor.create_reliable_client(
        supervisor,
        transport: [type: :mock, server_pid: self()],
    ...
    test "combining convenience functions with supervised clients", %{supervisor: supervisor} do
      ...
      assert {:ok, client} = ReliabilitySupervisor.create_reliable_client(
        supervisor,
        transport: [type: :mock, server_pid: self()]
      )
      ...
      protected_fn = Reliability.protect(fn -> {:ok, "protect_success"} end,
  ...
  describe "error handling and edge cases" do
    ...
    test "protect function handles exceptions in protected function" do
      ...
      end, name: String.to_atom("test_exception_#{:erlang.unique_integer()}"))
    ...
    test "with_retry handles various error formats" do
      ...
      assert {:error, {:retry_exhausted, :simple}} = Reliability.with_retry(fn -> {:error, :simple} end, max_attempts: 1)
      assert {:error, {:retry_exhausted, "string"}} = Reliability.with_retry(fn -> {:error, "string"} end, max_attempts: 1)
