...
| Stdio byte-mode framing and i18n payload tier | Backport (1.x patch or next minor) | Characterized correctness fix (GitHub #52; diagnosed in #41): both stdio transports read and write each device in its current mode through one framing owner; a payload corpus runs byte-exact through every transport with a locale matrix on the stdio subprocess test. No wire or API change; ASCII behavior identical. |
