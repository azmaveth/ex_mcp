defmodule ExMCP.Transport.Beam do
  @moduledoc """
  BEAM transport implementation that integrates with ExMCP.Protocol.

  This transport bridges the gap between the standard MCP Transport behavior
  (which expects JSON strings) and our BEAM transport components
  ...
  - **High Performance**: Uses optimized Frame, Connection, ZeroCopy, Batch modules
  ...
  This transport is the standard BEAM transport implementation:
  ...
  @doc """
  Establishes a BEAM transport connection.
  ...
  def connect(opts) do
    ...
    case Connection.start_link(connection_opts) do
      {:ok, connection} ->
        ...
        Logger.info("BEAM transport connected in #{mode} mode")
        {:ok, state}

      {:error, reason} ->
        Logger.error("Failed to establish BEAM connection: #{inspect(reason)}")
        {:error, reason}
    end
  end

  @doc """
  Sends a Protocol-encoded JSON message through the BEAM transport.

  The message (JSON string) is parsed, potentially batched, and sent through
  our connection with security validation and performance optimizations.
  """
  @impl true
  ...
  @doc """
  Receives a message from the BEAM transport.
  ...
  def receive_message(state) do
    # In our design, we use async message handling via owner process
    ...
    receive do
      ...
      {:connection_closed, _connection} ->
        Logger.info("BEAM connection closed")
  ...
  @doc """
  Closes the BEAM transport connection.
  ...
  def close(state) do
    ...
    Logger.info("BEAM transport closed")
    :ok
  end

  @doc """
  Checks if the BEAM transport connection is still active.
  """
  @impl true
  ...
  @doc """
  Accept a server connection (legacy compatibility function).

  This is called by ExMCP.Server when using transport: :beam.
  It wraps connect/1 with server mode enabled.
  """
  def accept(opts) do
    # Add server mode to opts and call connect
    server_opts = Keyword.put(opts, :mode, :server)
    connect(server_opts)
  end
