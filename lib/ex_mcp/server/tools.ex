defmodule ExMCP.Server.Tools do
  ...
  defmacro tool(name, description \\ nil, do: block) do
    quote do
      ExMCP.Server.Tools.__tool__(__MODULE__, unquote(name), unquote(description), unquote(Macro.escape(block)))
    end
  end
  ...
  def __tool__(module, name, description, block) do
    ...
    tool_def = build_tool_definition(name, final_description, params, annotations, 
  ...
  defp generate_schema_from_params(params) do
    properties = Enum.reduce(params, %{}, fn param, acc ->
  ...
  defp evaluate_ast_value({:%, _, [{:__aliases__, _, _}, {:%{}, _, _}]} = ast), do: evaluate_ast_map(ast)
